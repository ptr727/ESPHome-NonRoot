...
**ESPHome-NonRoot** builds and publishes a single multi-arch Docker image ([`ptr727/esphome-nonroot`](https://hub.docker.com/r/ptr727/esphome-nonroot)) that runs [ESPHome](https://esphome.io/) plus the `esphome-device-builder` dashboard as a non-root user, layered onto a `python:3.14-slim` base. It ships a single release target: the Docker image; consumers pull from Docker Hub on their own cadence. There is no application source in this repo - the Dockerfile is the only build input.
...
**The portability test decides where.** Anything an agent on a *different machine* would need to know goes in this file (or `WORKFLOW.md` / `CODESTYLE.md`) and is committed. Anything that is machine-specific or purely historic - local paths, host quirks, this-environment-only setup, or the story of how something came to be - goes in an untracked local agent file or agent memory, never here. The test is not "is it true?" but "would a fresh agent elsewhere be wrong without it?"
...
- **`upstream-version.json` is the shipped upstream pin, bot-maintained.** The tracker rewrites the whole file each run (sorted `esphome` + `device_builder`), so it carries no inline comment and is bot-maintained, not hand-edited. A new pin landing on `main` is what fires the publisher; the build reads it for the image tag and the `ESPHOME_VERSION` / `DEVICE_BUILDER_VERSION` build-args.
...
- Don't put release-bump magnitude in the title - no "minor", "patch", "release vX.Y.Z", etc. Nerdbank.GitVersioning computes the next release version from `version.json` + git history. Dependency versions in dependency-bump titles are fine and expected.
...
Bump docker/build-push-action from X to Y
...
- State the non-obvious *why*, not what the code already shows. No historic or design narrative, no rule citations - governance lives in this file, not echoed inline. **No cross-project references to unrelated repositories** - do not justify a design by another of our repos or point at a sibling project's convention. A link to upstream software this code works around is *not* a cross-project reference in that sense: where the non-obvious *why* is a specific upstream behavior, cite it (issue, docs anchor, or source line), because without it the workaround reads as unmotivated.
...
- `Docker/` - the multi-arch container build: [`Dockerfile`](./Docker/Dockerfile) (multi-stage, `python:3.14-slim` base, builds ESPHome wheels and runs as non-root), `entrypoint/` scripts, `Compose.yml`, and the static `README.md` published to Docker Hub.
- `.github/workflows/` - the branch-scoped CI/CD pipeline (see [`WORKFLOW.md`](./WORKFLOW.md)) plus the upstream-version tracker and the upstream-dependency watcher; `.github/compile-test/` - the ESPHome configurations the publisher's `compile-test` job builds inside the image before it is pushed, one per execution environment (Xtensa ESP-IDF, RISC-V ESP-IDF, wired Ethernet, PlatformIO/Arduino). Hand-authored and checked in, each naming the upstream ESPHome project it is modeled on; nothing is fetched or generated at run time, so the job cannot go red on someone else's change; **`repo-config/`** - the branch rulesets, settings, and secret audit as code.
- `.devcontainer/` - the single ESPHome runtime devcontainer.
- Docs - [`README.md`](./README.md) (also the GitHub-side overview), [`Docker/README.md`](./Docker/README.md) (the Docker Hub overview), [`HISTORY.md`](./HISTORY.md), this file, [`WORKFLOW.md`](./WORKFLOW.md), [`CODESTYLE.md`](./CODESTYLE.md), and [`.github/copilot-instructions.md`](./.github/copilot-instructions.md).
- Config - [`.editorconfig`](./.editorconfig), [`.gitattributes`](./.gitattributes), [`.markdownlint-cli2.jsonc`](./.markdownlint-cli2.jsonc), [`cspell.json`](./cspell.json), and [`ESPHome-NonRoot.code-workspace`](./ESPHome-NonRoot.code-workspace).
- [`version.json`](./version.json) - NBGV version floor (drives the GitHub release tag and `LABEL_VERSION`; independent of the ESPHome image tag).
- [`upstream-version.json`](./upstream-version.json) - the committed upstream version pins (`esphome` + `device_builder`) as a sorted JSON object, maintained by the tracker and read by the Docker build for the image tag + the `ESPHOME_VERSION` / `DEVICE_BUILDER_VERSION` build-args. Sits beside `version.json` (both are root-level version sources the build reads), not under `.github/` (which holds GitHub-platform config).
- [`upstream-dependency.json`](./upstream-dependency.json) - a snapshot of the apt package list ESPHome's own base image installs, maintained by the upstream-dependency watcher. **Not a shipped input**: no build step reads it, and it exists only so a change upstream opens a review PR. Deciding whether a package upstream adds belongs in this image is a human call - the watcher's PR is deliberately not auto-merged.

## Shared Contract Files

This repo owns its CI/CD outright: the branch-scoped, one-branch-per-run model specified in [`WORKFLOW.md`](./WORKFLOW.md) is the contract, and there is no inherited or upstream workflow model to defer to. A handful of cross-cutting files - this `AGENTS.md` (the "PR Review Etiquette" section especially), [`.github/copilot-instructions.md`](./.github/copilot-instructions.md), [`.markdownlint-cli2.jsonc`](./.markdownlint-cli2.jsonc), [`.editorconfig`](./.editorconfig), and [`.gitattributes`](./.gitattributes) - are kept converged across the maintainer's repos; keep them converged rather than re-inventing them, and raise a needed change with the maintainer so it lands everywhere. Everything else, including every workflow under [`.github/workflows/`](./.github/workflows/), is owned here and answers to `WORKFLOW.md` alone.

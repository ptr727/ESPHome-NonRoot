...
- `.github/workflows/` - the branch-scoped CI/CD pipeline (see [`WORKFLOW.md`](./WORKFLOW.md)) plus the upstream-version tracker; **`repo-config/`** - the branch rulesets, settings, and secret audit as code.
...
This repo is **derived from [`ptr727/ProjectTemplate`](https://github.com/ptr727/ProjectTemplate)**, but its CI/CD is the branch-scoped, one-branch-per-run model specified in [`WORKFLOW.md`](./WORKFLOW.md), not the template's older two-phase model. The cross-cutting contract carried from the template - this `AGENTS.md` (the "PR Review Etiquette" section especially), [`.github/copilot-instructions.md`](./.github/copilot-instructions.md), [`.markdownlint-cli2.jsonc`](./.markdownlint-cli2.jsonc), [`.editorconfig`](./.editorconfig), and [`.gitattributes`](./.gitattributes) - is shared byte-for-byte with the sibling Docker repos and the canonical sources; keep it converged rather than re-inventing it. The `build-docker-task.yml` leaf and the upstream-version tracker entry-point ([`check-upstream-version.yml`](./.github/workflows/check-upstream-version.yml), with the ESPHome/device-builder resolver) are owned here; everything else follows `WORKFLOW.md`.

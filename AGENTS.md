...
- **Bots (Dependabot and the upstream-version tracker) target both `main` and `develop` in parallel.** [`.github/dependabot.yml`](./.github/dependabot.yml) duplicates every ecosystem entry (one per branch), and [`.github/workflows/check-upstream-version.yml`](./.github/workflows/check-upstream-version.yml) calls the carried [`check-upstream-version-task.yml`](./.github/workflows/check-upstream-version-task.yml), which matrixes over both branches, opening rolling bump PRs on branches `upstream-version-main` and `upstream-version-develop`. Each branch absorbs its own bot PRs independently, so neither falls behind, and the forward-only rule still holds (nothing is back-merged from main to develop - both branches receive their updates directly). The merge-bot ([`.github/workflows/merge-bot-pull-request.yml`](./.github/workflows/merge-bot-pull-request.yml)) dispatches `--squash` or `--merge` from each PR's base ref via a `case` statement so the form matches the ruleset on either base. Dependabot **security** PRs (CVE-driven) always open against the repo default branch (`main`) regardless of `target-branch` - the same `case` statement covers them.
- **Maintainer-pushed commits on a bot PR auto-disable auto-merge.** The merge-bot's `merge-dependabot` and `merge-upstream-version` jobs only fire on `opened` / `reopened` events (auto-merge is enabled exactly once per PR). When a maintainer pushes commits to a bot's branch (a `synchronize` event with an actor that isn't the same bot), the merge-bot's `disable-auto-merge-on-maintainer-push` job fires and calls `gh pr merge --disable-auto`. The maintainer's commits stay in the PR but won't auto-merge with the bot's content; re-enable auto-merge manually (`gh pr merge --auto <PR>` or the GitHub UI) when ready.
...
- **No-op republish guarantee.** A weekly/dispatch publish where NBGV `SemVer2` is **unchanged** (no new commit since the last publish) re-pushes **nothing** to GitHub Releases - the `github-release` job's `release-exists` check skips the create step (it keys on the version string). **Docker always re-pushes** by design: it picks up upstream base-image refreshes (`python:3.14-slim`) and is how a new ESPHome version reaches Docker Hub. Boundary: `version.json` has **no `pathFilters`**, so *any* commit - including a CI/workflow-only or docs-only change - advances the NBGV git height and therefore `SemVer2`, and the next publish *does* create a fresh release for it even when the image is otherwise unchanged. This is accepted NBGV behavior; `pathFilters` are intentionally not added.
...
- `.github/` - workflows (the two-layer release pipeline + the upstream-version tracker), Dependabot, and Copilot instructions.
...
- `upstream-version.json` - the committed upstream version pins (`esphome` + `device_builder`) as a sorted JSON object, maintained by the tracker and read by the Docker build for the image tag + the `ESPHOME_VERSION` / `DEVICE_BUILDER_VERSION` build-args. The tracker rewrites the whole file each run, so it carries no inline comment and is bot-maintained, not hand-edited. Sits beside `version.json` (both are root-level version sources the build reads), not under `.github/` (which holds GitHub-platform config).

## Template Lineage and Re-Syncing

This repo is **derived from [`ptr727/ProjectTemplate`](https://github.com/ptr727/ProjectTemplate)** (develop branch). It is past initial bootstrap; the ongoing concern is keeping the carried contract in sync (below). The orchestration workflows (`publish-release.yml`, `get-version-task.yml`, `build-datebadge-task.yml`, `publish-docker-readme-task.yml`, `merge-bot-pull-request.yml`, the `get-version` + `github-release` jobs in `build-release-task.yml`, the aggregator shape of `test-pull-request.yml`), the carried `check-upstream-version-task.yml`, and the mandatory shared files are synced from the template; the `build-docker-task.yml` leaf and the upstream-version tracker entry-point (`check-upstream-version.yml`, which calls that task with the ESPHome/device-builder resolver) are owned here.
...
These are the intentional, documented deviations from the template for this repo. Anything not listed here is expected to match the template's carried form.

- **Docker-only; no .NET, Python, or other language source tree.** The template's language-specific build leaves (`build-executable-task.yml`, `build-nugetlibrary-task.yml`, `build-pypilibrary-task.yml`), the `run-codegen-*` workflows, and the `unit-test` / `merge-codegen` jobs are **not** carried - this repo has no compiled code or codegen. `test-pull-request.yml` keeps only `changes` (Docker filter) -> `smoke-build` -> `check-workflow-status` plus the terminal `cleanup-artifacts` job; `merge-bot-pull-request.yml` carries `merge-dependabot`, `merge-upstream-version`, and `disable-auto-merge-on-maintainer-push` but omits `merge-codegen`. The shared files that don't apply to a Docker-only repo (the inert `[*.cs]` block in `.editorconfig`, the .NET sections of `CODESTYLE.md`) are **carried whole**, not trimmed - this repo keeps every section of a carried file even when a section is inert here.
- **`check-upstream-version` instead of codegen.** This repo tracks upstream releases (the `esphome` and `device_builder` versions) rather than generating code. `check-upstream-version.yml` (entry-point, owned here) calls the carried `check-upstream-version-task.yml` with the ESPHome/device-builder resolver; it rewrites `upstream-version.json` and opens an `upstream-version-*` PR that `merge-bot-pull-request.yml`'s `merge-upstream-version` job auto-merges. There is no `run-codegen-*` workflow.

...
`CODEGEN_APP_CLIENT_ID` / `CODEGEN_APP_PRIVATE_KEY` for the merge-bot and the upstream-version tracker), and *notes* (best-effort) whether a
GitHub App is installed - a precise check needs app-level auth, so the App-installation check does not fail
the audit.
...
```sh
REPO=ptr727/ESPHome-NonRoot ./repo-config/configure.sh apply   # sync live rulesets + settings + security
REPO=ptr727/ESPHome-NonRoot ./repo-config/configure.sh check   # confirm no drift
```
